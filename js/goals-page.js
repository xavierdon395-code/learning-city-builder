(function () {
  ...
  var goalsRefreshSeq = 0;

  function asObject(value) {
    return value && typeof value === "object" ? value : {};
  }

  function setGoalsFallbackState(message) {
    ...
    if (emptyEl) {
      ...
      emptyEl.textContent = message || "目标加载失败，请稍后重试";
    }
  }

  function refresh(user) {
    var uid = user.uid;
    var seq = ++goalsRefreshSeq;
    Promise.all([AppData.loadProfile(uid), AppData.loadGoals(uid), AppData.loadAllTasks(uid)])
      .then(function (res) {
        if (seq !== goalsRefreshSeq) return;
        res = res || [];
        var profile = asObject(res[0]);
        var goalsRaw = asObject(res[1]);
        var allTasks = asObject(res[2]);
      ...
      .catch(function (e) {
        ...
        setGoalsFallbackState("目标加载失败，请稍后重试");
      });
  }
