...
const PDFExport = (function () {
  ...
  async function exportPDF(options) {
    ...
    doc.addPage();
    await renderGoalsPageAsImage(doc, dataOpts);
  ...
  /**
   * 阶段 2：嵌入 Noto Sans SC (GBK 子集) 让 PDF 支持中文。
   * Font globals injected by font-noto-sans-sc.js -> window.NOTO_SANS_SC_BASE64
   * 注册只发生一次(由 jsPDF 的 getFontList 检查)。
   */
  async function ensureChineseFont(doc) {
    var b64 = (typeof window !== "undefined") ? window.NOTO_SANS_SC_BASE64 : null;
    if (!b64) {
      console.warn("[PDF] Noto Sans SC base64 not loaded; falling back to Helvetica (中文将显示为方块)");
      doc.setFont("NotoSansSC", "normal");
      return;
    }
    try {
      var fontList = doc.getFontList ? doc.getFontList() : {};
      ...
      console.error("[PDF] Failed to register Noto Sans SC, falling back to Helvetica", err);
      doc.setFont("NotoSansSC", "normal");
    }
  }
  ...
  async function renderGoalsPageAsImage(doc, options) {
    options = options || {};
    ...
    var scale = 4;
    var pageWmm = LAYOUT.page_width;
    var pageHmm = LAYOUT.page_height;
    var W = Math.round(pageWmm * scale);
    var H = Math.round(pageHmm * scale);
    var margin = Math.round(LAYOUT.margin_x * scale);
    ...
    function makeCanvas(isCont) {
      ...
      ctx.font = "700 " + Math.round(22 * scale) + "px -apple-system, BlinkMacSystemFont, 'PingFang SC', 'Noto Sans SC', sans-serif";
      ctx.fillText(isCont ? "GOALS PROGRESS (cont.)" : "GOALS PROGRESS", margin, Math.round(38 * scale));

      if (!isCont) {
        ctx.fillStyle = COLORS.text_secondary;
        ctx.font = "400 " + Math.round(10 * scale) + "px -apple-system, BlinkMacSystemFont, 'PingFang SC', 'Noto Sans SC', sans-serif";
        ctx.fillText("Active goals and their completion status", margin, Math.round(47 * scale));
      }

      ctx.strokeStyle = COLORS.accent;
      ctx.lineWidth = Math.max(1, Math.round(0.5 * scale));
      ctx.beginPath();
      ctx.moveTo(margin, Math.round((isCont ? 45 : 53) * scale));
      ctx.lineTo(margin + Math.round(25 * scale), Math.round((isCont ? 45 : 53) * scale));
      ctx.stroke();
    ...
    function drawFooter(ctx) {
      ...
      ctx.lineWidth = Math.max(1, Math.round(0.3 * scale));
      ctx.beginPath();
      ctx.moveTo(margin, H - Math.round(20 * scale));
      ctx.lineTo(W - margin, H - Math.round(20 * scale));
      ctx.stroke();

      ctx.fillStyle = COLORS.text_muted;
      ctx.font = "400 " + Math.round(8 * scale) + "px -apple-system, BlinkMacSystemFont, 'PingFang SC', 'Noto Sans SC', sans-serif";
      ctx.fillText("GOALS", margin, H - Math.round(14 * scale));

      ctx.textAlign = "right";
      ctx.fillText("Generated by Lumi · " + formatDate(new Date()), W - margin, H - Math.round(14 * scale));
      ctx.textAlign = "left";
    }
    ...
    var pages = [];
    var page = makeCanvas(false);
    var ctx = page.ctx;
    var y = Math.round(75 * scale);
    var maxY = H - Math.round(42 * scale);

    if (goals.length === 0) {
      ctx.textAlign = "center";
      ctx.fillStyle = COLORS.text_muted;
      ctx.font = "500 " + Math.round(13 * scale) + "px -apple-system, BlinkMacSystemFont, 'PingFang SC', 'Noto Sans SC', sans-serif";
      ctx.fillText("NO GOALS YET", W / 2, Math.round(130 * scale));
      ctx.font = "400 " + Math.round(10 * scale) + "px -apple-system, BlinkMacSystemFont, 'PingFang SC', 'Noto Sans SC', sans-serif";
      ctx.fillText("Start your first goal in the app to see progress here", W / 2, Math.round(140 * scale));
      ctx.textAlign = "left";
      drawFooter(ctx);
      ...
      goals.forEach(function (row, idx) {
        ...
        ctx.font = "600 " + Math.round(13 * scale) + "px -apple-system, BlinkMacSystemFont, 'PingFang SC', 'Noto Sans SC', sans-serif";
        var maxNameW = W - margin * 2 - Math.round(46 * scale);
        var lineHeight = Math.round(6.4 * scale);

        var previewLines = [];
        var cur = "";
        String(name).split("").forEach(function (ch) {
          var t = cur + ch;
          if (ctx.measureText(t).width > maxNameW && cur) {
            previewLines.push(cur);
            cur = ch;
          } else {
            cur = t;
          }
        ...
        var metaParts = [];
        if (goal.type) metaParts.push(String(goal.type));
        if (goal.deadline) metaParts.push("DUE " + String(goal.deadline));

        var rowH = previewLines.length * lineHeight + Math.round(28 * scale) + (metaParts.length ? Math.round(8 * scale) : 0);

        if (y + rowH > maxY) {
          drawFooter(ctx);
          pages.push(page.canvas);
          page = makeCanvas(true);
          ctx = page.ctx;
        ...
        ctx.fillStyle = COLORS.text_primary;
        ctx.font = "600 " + Math.round(13 * scale) + "px -apple-system, BlinkMacSystemFont, 'PingFang SC', 'Noto Sans SC', sans-serif";
        var usedH = drawWrapped(ctx, name, margin, y, maxNameW, lineHeight);

        ctx.textAlign = "right";
        ctx.fillStyle = COLORS.accent;
        ctx.font = "600 " + Math.round(13 * scale) + "px -apple-system, BlinkMacSystemFont, 'PingFang SC', 'Noto Sans SC', sans-serif";
        ctx.fillText(String(progPct) + "%", W - margin, y);
        ctx.textAlign = "left";

        var barY = y + usedH + Math.round(5 * scale);
        var barW = W - margin * 2;
        var barH = Math.max(3, Math.round(2 * scale));
        ...
        if (progPct > 0) {
          ctx.fillStyle = COLORS.accent;
          ctx.fillRect(margin, barY, Math.round(barW * progPct / 100), barH);
        }

        if (metaParts.length > 0) {
          ctx.fillStyle = COLORS.text_muted;
          ctx.font = "400 " + Math.round(8 * scale) + "px -apple-system, BlinkMacSystemFont, 'PingFang SC', 'Noto Sans SC', sans-serif";
          ctx.fillText(metaParts.join("  ·  "), margin, barY + barH + Math.round(6 * scale));
        }
